...
for i in train_vec:
    time.sleep(600)
    df_output = simulation(models_list=models,client=client,users_list=users,num_of_users=10,train_ratio=i,
...
df_output = simulation(data_set=data_set, models_list=models, client=client, users_list=users, num_of_users=len(users),
                        train_ratio=0.6, recomm_to_test=3, directory=directory)
...
print(df_output)
df_output.to_excel('simulations.xlsx')
